...
class IronBidPDF(FPDF):
    def header(self):
        if getattr(self, 'logo_data', None):
            self.image(self.logo_data, x=10, y=10, w=40)
        self.set_xy(120, 10)
...
@st.cache_data
def generate_bid(project_name, location, client_name, bid_total, plumbing_fixtures, terms, logo_data=None):
    """
    Generates a professional bid PDF.
    """
    today = date.today().strftime("%B %d, %Y")
    pdf = IronBidPDF()
    pdf.logo_data = logo_data
    pdf.add_page()
    pdf.set_font("Helvetica", size=11)
...
# Streamlit UI
st.title("Iron Plumbing Bid Generator (Optimized)")
...
# Bid Details Form
project = st.text_input("Project Name", value="New Plumbing Project")
location = st.text_input("Project Location", value="123 Main St, Anytown, USA")
client = st.text_input("Prepared For (GC/Client)", value="General Contractor")
bid_total = st.number_input("Total Bid Amount", min_value=0.0, step=100.0, value=10000.0)

# Plumbing Fixtures Input
st.subheader("Plumbing Fixtures")
fixtures = st.text_area("List each fixture on a new line", value="Toilet\nSink\nShower")
...
if st.button("Generate Bid PDF"):
    ...
    if uploaded_logo:
        ...
        image = image.resize((200, 200))  # Resize for consistency
        logo_data = BytesIO()
        image.save(logo_data, format="PNG")
        logo_data.seek(0)

    # Generate PDF
    try:
        pdf_buffer = generate_bid(project, location, client, bid_total, fixture_list, terms_list, logo_data)
        st.download_button("Download Bid PDF", pdf_buffer, file_name="Bid_Document.pdf", mime="application/pdf")
